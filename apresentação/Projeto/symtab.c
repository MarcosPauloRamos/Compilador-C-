...
void st_insert( char * name, int lineno, int op, char* escopo, dataTypes DType, IDTypes IType )
{
  ...
  if ( l == NULL || (op != 0 && l->escopo != escopo && l->IType != FUN)) /* variable not yet in table */
  {
    ...
    fprintf(listing,"Erro Semantico: Nome da variavel %s já utilizada como nome de função.[%d]\n",name, lineno);
    Error = TRUE;
  }
  else if( l->escopo == escopo && op != 0)
  {
    fprintf(listing,"Erro Semantico: Variavel %s já declarada neste escopo.[%d]\n",name, lineno);
    Error = TRUE;
  }
    ...
    if(l == NULL){
      fprintf(listing,"Erro Semantico: Variavel %s não declarada neste escopo.[%d]\n",name, lineno);
      Error = TRUE;
    }
  }
  else if(op == 0)
